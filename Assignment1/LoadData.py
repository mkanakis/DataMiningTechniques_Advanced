...
def normalize(df):
    ...
    for feature_name in df.columns:
        if feature_name not in ['mood', 'mood_', 'circumplex.valence', 'circumplex.arousal', 'id', 'datepart',
                                'weekday']:
        ...
        for feature_name in persondf.columns:
            ...
            if feature_name not in ['id', 'datepart', 'call', 'sms', 'weekday']:
                ...
                persondf = persondf.drop(feature_name, 1)
                                      ...
                                      'weekday']:  # looking at the sum instead of the mean of the previous days for sms and call
                # persondf[str(feature_name)+'PrevDay'] = persondf[feature_name].shift(1)
                # persondf[str(feature_name)+'PrevDay'] = persondf[str(feature_name)+'PrevDay'].fillna(0)
...
tuned_parameters_tree = [
    {'min_samples_leaf': range(35, 46, 5), 'max_depth': range(45, 66, 5), 'max_features': range(50, 56, 1)}]
tuned_parameters_forest = [
    {'n_estimators': range(20, 31, 5), 'min_samples_leaf': range(10, 31, 5), 'max_depth': range(10, 21, 5),
     'max_features': range(45, 52, 2)}]
params = {'eta': 0.1, 'seed': 0, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'reg:linear', 'max_depth': 3,
          'min_child_weight': 1}
tuned_parameters_xgb = [{'max_depth': range(3, 11, 1), 'n_estimators': 300, 'learning_rate': range(0.01, 0.21, 0.01),
                         'colsample_bytree': range(0.4, 0.91, 0.1), 'eta': range(0.05, 0.31, 0.01),
                         'subsample': range(0.5, 0.96, 0.05), 'eval_metric': 'rmse'}]

models = [(tuned_parameters_tree, tree_, 'tree'), (tuned_parameters_forest, forest_, 'forest'),
          (tuned_parameters_xgb, xgb, 'xgb')]
