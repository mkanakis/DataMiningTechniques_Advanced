        ...
        for feature_name in persondf.columns:
            ...
            if feature_name not in ['id', 'datepart', 'call', 'sms', 'weekday']:
                ...
                persondf[str(feature_name) + 'Gradient'] = np.gradient(
                    persondf[str(feature_name) + 'PrevDay'].rolling(7).mean())
...
print('\tMean MSE baseline:', round(sum(mses_b) / len(mses_b), 3), '\tMean MSE model:',
      round(sum(mses_m) / len(mses_m), 3))
t, p = ttest_ind(mses_t, mses_m, equal_var=False)
print('\nT-test on mses of the models (training vs testing)||t:', t, '\tp:', p)
print('\tMean MSE training:', round(sum(mses_t) / len(mses_t), 3), '\tMean MSE testing:',
      round(sum(mses_m) / len(mses_m), 3))
